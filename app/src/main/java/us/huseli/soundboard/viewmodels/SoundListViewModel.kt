...
class SoundListViewModel : ViewModel() {
    ...
    fun update(sound: Sound) = viewModelScope.launch(Dispatchers.IO) {
        repository.update(sound)
    }

    fun update(sounds: List<Sound>) = viewModelScope.launch(Dispatchers.IO) {
        sounds.forEachIndexed { index, sound -> sound.order = index }
